...
// functionto generate unique seat number
...
const reserveTicket = async (req, res) => {
    ...
    if (!name || !email || !event_date) {
        return res.status(400).json({
            message: 'email, name and event date are required.'
        });
    }
    ...
    if (!event.length) {
        return res.status(404).json({
            message: 'No event found on the specified date.'
        });
    }
    ...
    if (userInEvent.length > 0) {
        return res.status(404).json({
            message: 'User already has a ticket for the event.'
        });
    }
    ...
    return res.status(201).json({
        message: 'Your ticket is booked successful!',
        reservation_data: newReservation
    });
}
...
const getTickets = async (req, res) => {
    ...
    return res.json({
        ticket_data: userReservations[0],
        message: "Ticket data fetch succesfully."
    });
}
...
const cancelTicket = async (req, res) => {
    ...
    if (userReservationIndex === -1) {
        return res.status(404).json({
            message: 'No reservation found for the provided email and event date.'
        });
    }
...
const updateTicket = async (req, res) => {
    ...
    res.json({
        ...
        modified_ticket: reservations[userReservationIndex]
    });
}
